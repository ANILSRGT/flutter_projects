...
void main() {
  setupLocator();
  runApp(ChangeNotifierProvider(builder: (context) => MyThemeViewModel(), child: MyApp()));
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<MyThemeViewModel>(
      builder: (context, MyThemeViewModel myThemeViewModel, child) => MaterialApp(
        title: 'Weather App',
        debugShowCheckedModeBanner: false,
        theme: myThemeViewModel.myTheme.tema,
        home: ChangeNotifierProvider<WeatherViewModel>(
            builder: (context) => locator<WeatherViewModel>(), child: WeatherApp()),
      ),
    );
  }
}
